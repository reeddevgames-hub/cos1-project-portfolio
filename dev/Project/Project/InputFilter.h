...
public:
	...
	static int SafeInteger(const char* promptMessage, const char* errorMessage);
	...
	static double SafePositiveDouble(const char* promptMessage, const char* errorMessage);

	// Will be used to filter out any invalid inputs and loop till a string input is accepted. 
	static std::string SafeString(const char* promptMessage, const char* errorMessage);
};
